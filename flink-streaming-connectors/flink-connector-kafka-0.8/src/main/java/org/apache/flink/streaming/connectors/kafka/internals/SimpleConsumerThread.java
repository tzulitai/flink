...
class SimpleConsumerThread<T> extends Thread {
	...
	public void run() {
		...
		try {
			...
			getEarliestOrLatestOffsetsFromKafka(consumer, partitions);
			...
			while (running) {
				...
				if (newPartitions != null) {
					...
					getEarliestOrLatestOffsetsFromKafka(consumer, newPartitions);
				...
				if (fetchResponse.hasError()) {
					...
					if (partitionsToGetOffsetsFor.size() > 0) {
						...
						getSpecificTimeOffsetsFromKafka(consumer, partitionsToGetOffsetsFor, invalidOffsetBehavior);
	 ...
	 */
	private static void getSpecificTimeOffsetsFromKafka(
	 ...
	 * @param partitions The list of partitions we need offsets for
	 */
	private static void getEarliestOrLatestOffsetsFromKafka(
			SimpleConsumer consumer,
