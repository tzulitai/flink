...
/**
 * Factory for different kinds of fake Kinesis behaviours using the {@link IKinesisProxy} interface.
 */
public class FakeKinesisBehavioursFactory {
	...
	public static IKinesisProxy noShardsFoundForRequestedStreamsBehaviour() {

		return new IKinesisProxy() {
			@Override
			public GetShardListResult getShardList(List<String> streamNames) {
	...
	public static IKinesisProxy nonReshardedStreamsBehaviour(Map<String,Integer> streamsToShardCount) {
	...
	public static IKinesisProxy totalNumOfRecordsAfterNumOfGetRecordsCalls(final int numOfRecords, final int numOfGetRecordsCalls) {
		return new SingleShardEmittingFixNumOfRecordsKinesis(numOfRecords, numOfGetRecordsCalls);
	}

	private static class SingleShardEmittingFixNumOfRecordsKinesis implements IKinesisProxy {
	...
	private static class NonReshardedStreamsKinesis implements IKinesisProxy {
		...
		public GetShardListResult getShardList(List<String> streamNames) {
			...
			for (Map.Entry<String, List<KinesisStreamShard>> streamsWithShards : streamsWithListOfShards.entrySet()) {
				result.addRetrievedShardsToStream(streamsWithShards.getKey(), streamsWithShards.getValue());
			}
			return result;
		}
				...
				for (KinesisStreamShard shard : streamsWithShards.getValue()) {
					if (streamNamesWithLastSeenShardIds.get(streamName) == null) {
						...
						if (KinesisStreamShard.compareShardIds(
							shard.getShardId(), streamNamesWithLastSeenShardIds.get(streamName)) > 0) {
							result.addRetrievedShardToStream(streamName, shard);
						}
					}
				}
