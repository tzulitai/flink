...
final class OptionSerializerBuilder[A](private var elemSerializerBuilder: TypeSerializerConfiguration[A])
  ...
  override def build(): TypeSerializer[Option[A]] = {
    new OptionSerializer[A](elemSerializerBuilder.build())
  }

  override def getVersion: Int = OptionSerializerBuilder.VERSION
}

object OptionSerializerBuilder {
  val VERSION = 1
}
