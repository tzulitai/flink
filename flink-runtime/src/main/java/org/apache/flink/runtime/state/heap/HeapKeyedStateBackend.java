...
import org.apache.flink.runtime.state.UnresolvableStateException;
...
public class HeapKeyedStateBackend<K> extends AbstractKeyedStateBackend<K> {
	...
	private <N, V> StateTable<K, N, V> tryRegisterStateTable(
		...
		if (stateTable == null) {
			...
			try {
				stateTable.getMetaInfo().resolve(newMetaInfo);
			} catch (UnresolvableStateException e) {
				throw new RuntimeException("The previous registered state meta info cannot be resolved. " +
					"Original meta info was " + stateTable.getMetaInfo() + ", trying to resolve with " + newMetaInfo, e);
			}
		}
		return stateTable;
	}
	...
	public  RunnableFuture<KeyedStateHandle> snapshot(
		...
		List<RegisteredBackendStateMetaInfo<?, ?>> metaInfoList = new ArrayList<>(stateTables.size());
	...
	private void restorePartitionedState(Collection<KeyedStateHandle> state) throws Exception {
		...
		for (KeyedStateHandle keyedStateHandle : state) {
			...
			try {
				...
				List<RegisteredBackendStateMetaInfo<?, ?>> metaInfoList = serializationProxy.getNamedStates();

				for (RegisteredBackendStateMetaInfo<?, ?> restoredMetaInfo : metaInfoList) {
					...
					if (null == stateTable) {
						stateTable = newStateTable(restoredMetaInfo);
