...
/**
 * Utility methods for {@link TypeSerializer}.
 */
@Internal
public class TypeSerializerUtil {
	 ...
	 * @param serializerConfigSnapshot the serializer configuration snapshot to write; {@code null} is handled
	 *
	 * @throws IOException
	 */
	 ...
	 *
	 * @param out the data output view
	 * @param serializerConfigSnapshots the serializer configuration snapshots to write; {@code null} is handled
	 *
	 * @throws IOException
	 */
	...
	static class TypeSerializerConfigSnapshotProxy extends VersionedIOReadableWritable {
		...
		public void read(DataInputView in) throws IOException {
			...
			if (in.readBoolean()) {
				...
				try {
					serializerConfigSnapshotClass = InstantiationUtil.deserializeObject(
							new DataInputViewStream(in), userCodeClassLoader);
				} catch (ClassNotFoundException e) {
					throw new IOException(
							"Could not find requested TypeSerializerConfigSnapshot class in classpath.", e);
				}
