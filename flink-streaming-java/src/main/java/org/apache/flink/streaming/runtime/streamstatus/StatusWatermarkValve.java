 ...
 * implementation of {@link ValveOutputHandler}, which is used by the valve only when it determines a new watermark or
...
public class StatusWatermarkValve {
	...
	/**
	 * Feed a {@link StreamStatus} into the valve. If the input triggers the valve to output a new Stream Status,
	 * {@link ValveOutputHandler#handleWatermark(Watermark)} will be called to process the new Watermark.
	 *
	...
	public void inputStreamStatus(StreamStatus streamStatus, int channelIndex) {
		...
		if (streamStatus.isIdle() && channelStatuses[channelIndex].streamStatus.isActive()) {
			...
			// if all input channels of the valve is now idle, we need to output an idle stream
