...
public abstract class AbstractStreamOperator<OUT>
	...
	private class CountingOutput implements Output<StreamRecord<OUT>> {
		 ...
		 *
		 * NOTE: This currently only works if the watermark generating
		 * implementation is an one-input operator.
		 */
		private StreamStatus currentOverallStatus;

		private CountingOutput(final Output<StreamRecord<OUT>> output, final Counter counter) {
			this.output = checkNotNull(output);
			this.numRecordsOut = checkNotNull(counter);

			this.currentOverallStatus = StreamStatus.ACTIVE;
		}

		@Override
		public void emitWatermark(Watermark mark) {
			if (currentOverallStatus.isActive()) {
		...
		public void emitStreamStatus(StreamStatus streamStatus) {
			this.currentOverallStatus = streamStatus;
	...
	//
	//  NOTE:
	//  Since Watermark processing methods may be overriden by timestamp
	//  assigner / watermark emitting operators to completely bypass the valves
	//  and generate watermarks, we need to rely on the base Stream Status
	//  processing methods at the abstract operator level to correctly determine
	//  whether the operator is actually marked idle and that the generated
	//  watermarks should be blocked.
	//  Therefore, it is important that concrete implementations do not override
	//  the Stream Status processing methods and let Stream Status elements
	//  bypass the valve.
	// ------------------------------------------------------------------------
	...
	private class WatermarkAdvancingValveOutputHandler implements StatusWatermarkValve.ValveOutputHandler {
		...
		private WatermarkAdvancingValveOutputHandler(
			...
			final Map<String, HeapInternalTimerService<?, ?>> timerServices) {
			this.output = checkNotNull(output);
		...
		public void handleWatermark(Watermark watermark) {
			try {
				// use the valve's output watermarks to advance the operator's current watermark
				for (HeapInternalTimerService<?, ?> service : timerServices.values()) {
					service.advanceWatermark(watermark.getTimestamp());
				}
			} catch (Exception e) {
				throw new RuntimeException("Exception occurred while advancing operator watermark: ", e);
			}
